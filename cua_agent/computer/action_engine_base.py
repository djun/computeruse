"""Shared action-routing engine.

OS adapters subclass :class:`SharedActionEngine`, build their platform drivers in
``__init__`` (setting ``self.hid_driver``, ``self.semantic_driver``,
``self.shell_driver``, ``self.accessibility_driver``, ``self.browser_driver``,
``self.display`` and ``self.vision_pipeline``), and implement the small set of
OS-specific hooks below. Everything else — policy gating, HITL, semantic/visual
target resolution, phantom-mode HID routing, waits, macros — is identical across
platforms and lives here.

OS-specific hooks:
  * ``_primary_modifier_key`` — "command" (macOS) vs "ctrl" (Windows)
  * ``_open_app`` — app launcher sequence (Spotlight vs Start menu)
  * ``_clipboard_read`` / ``_clipboard_write`` / ``_clipboard_clear`` — clipboard I/O
  * ``_platform_enrich`` — extra safety context (Windows foreground process/title)
  * ``_extra_hitl_reason`` — extra high-risk heuristics (Windows UAC/shell)
  * ``_execute_browser`` — browser dispatch (+ Windows cyborg fallback)
Class attributes ``POLICY_HITL_DEFAULT`` and ``DEFAULT_BROWSER_APP`` carry the
remaining per-OS defaults.
"""
...
class SharedActionEngine(ABC):
    """Routes actions through policy evaluation, semantic path, or HID injection."""

    # Per-OS defaults (overridden by adapters where they differ).
    POLICY_HITL_DEFAULT = "policy hitl"
    DEFAULT_BROWSER_APP = "Safari"

    # Optional dedicated grounder (set by adapters); resolves a target description
    # to coordinates when the accessibility tree and visual detector both miss.
    ...
    def _platform_enrich(self, enriched: dict) -> None:
        """Attach OS-specific safety context to an action in place. No-op by default."""
        return None

    def _extra_hitl_reason(self, action: dict) -> str:
        """Extra platform HITL heuristics. Empty (no extra gate) by default."""
        return ""
    ...
    def execute(self, action: dict) -> ActionResult:
        ...
        if decision.hitl_required:
            hitl_reason = decision.reason or self.POLICY_HITL_DEFAULT
        if not hitl_reason:
            hitl_reason = self._extra_hitl_reason(action)
        ...
        if action_type == "zoom":
            return self._handle_zoom(action)
    ...
    def _enrich_action(self, action: dict) -> dict:
        ...
        plus any platform-specific context via ``_platform_enrich``.
        """
        enriched = dict(action)
        self._platform_enrich(enriched)

        if enriched.get("execution") == "browser" and enriched.get("command") == "run_javascript":
            try:
                page_url = self.browser_driver.get_current_url(enriched.get("app_name", self.DEFAULT_BROWSER_APP))
                if page_url:
    ...
    def _handle_zoom(self, action: dict) -> ActionResult:
        """Re-capture a region at higher detail so the model can read small UI.

        The zoomed image is returned in metadata; coordinates for subsequent
        actions remain in the original logical screenshot space.
        """
        if not self.vision_pipeline:
            return ActionResult(success=False, reason="zoom requires a vision pipeline")
        box = action.get("region") or action.get("box")
        if not box:
            return ActionResult(success=False, reason="zoom requires region [x1,y1,x2,y2]")
        try:
            upscale = float(action.get("upscale", 2.0))
        except (TypeError, ValueError):
            upscale = 2.0
        try:
            image = self.vision_pipeline.capture_zoom(box, upscale=upscale)
        except Exception as exc:
            return ActionResult(success=False, reason=f"zoom failed: {exc}")
        return ActionResult(
            success=True,
            reason="captured zoom region",
            metadata={"zoom_image": image, "region": box, "grounding": "zoom"},
        )
    ...
    def _resolve_semantic_target(
        ...
        *,
        ...
        # Dedicated grounder (composed-agent pattern): resolve a described target to
        # coordinates when AX + visual detection both missed.
        grounder = getattr(self, "grounding_model", None)
        target_desc = hint_label or element_ref or hint_role
        if grounder is not None and getattr(grounder, "available", False) and target_desc and self.vision_pipeline:
            try:
                frame = self.vision_pipeline.capture_base64()
                coords = grounder.locate(frame, target_desc, self.display)
                if coords:
                    return {
                        "x": coords[0],
                        "y": coords[1],
                        ...
                        "grounding_source": "uitars",
                    }, "resolved via grounder model"
            except Exception as exc:
                self.logger.debug("grounder target resolution failed: %s", exc)
